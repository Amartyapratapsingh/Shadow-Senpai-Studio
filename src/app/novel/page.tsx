...
/**
 * Build audio fallback chain: selected voice → preset 2 → preset 3 → opposite provider default.
 * When primary hits rate limit, automatically tries next option.
 */
function getAudioFallbackChain(primaryProvider: AudioProvider, primaryVoice: string): { provider: AudioProvider; voice: string; apiKey: string }[] {
  const chain: { provider: AudioProvider; voice: string; apiKey: string }[] = [];
  const seen = new Set<string>();

  // 1. Primary selection
  const pk = getApiKey(primaryProvider === "gemini" ? "gemini" : "openai");
  if (pk) { const key = `${primaryProvider}:${primaryVoice}`; chain.push({ provider: primaryProvider, voice: primaryVoice, apiKey: pk }); seen.add(key); }

  // 2. Voice presets from Audio page (preset 2 and 3 as fallbacks)
  const presets = loadVoicePresets();
  for (const preset of presets) {
    if (!preset) continue;
    const key = `${preset.provider}:${preset.voice}`;
    if (seen.has(key)) continue;
    const k = getApiKey(preset.provider === "gemini" ? "gemini" : "openai");
    if (k) { chain.push({ provider: preset.provider, voice: preset.voice, apiKey: k }); seen.add(key); }
  }

  // 3. Opposite provider default (if not already in chain)
  const oppositeProvider: AudioProvider = primaryProvider === "openai" ? "gemini" : "openai";
  const oppKey = getApiKey(oppositeProvider === "gemini" ? "gemini" : "openai");
  if (oppKey) {
    const defaultVoice = oppositeProvider === "openai" ? "cedar" : "Kore";
    const key = `${oppositeProvider}:${defaultVoice}`;
    if (!seen.has(key)) chain.push({ provider: oppositeProvider, voice: defaultVoice, apiKey: oppKey });
  }

  return chain;
}

function isAudioRateLimitError(errorMsg: string): boolean {
  const lower = errorMsg.toLowerCase();
  return lower.includes("quota") || lower.includes("rate limit") || lower.includes("rate_limit") ||
    lower.includes("exceeded") || lower.includes("too many requests") || lower.includes("429") ||
    lower.includes("resource_exhausted") || lower.includes("retry in");
}
...
export default function NovelPage() {
  ...
  const resumePipeline = useCallback(async (saved: SavedPipeline) => {
    ...
    // ── Step 3: Resume audio — per-panel, only pending/errored ones, with fallback ──
    {
      const failedAudioPanels = saved.panels.map((p, i) => ({ ...p, idx: i })).filter(p => p.audioStatus !== "done");
      if (failedAudioPanels.length > 0) {
        const audioChain = getAudioFallbackChain(voice.provider as AudioProvider, voice.voice);
        if (audioChain.length > 0) {
          setAudioStatus("running");
          // Ensure blob array matches panel count
          if (audioBlobsRef.current.length !== saved.panels.length) audioBlobsRef.current = new Array(saved.panels.length).fill(null);

          for (const pp of failedAudioPanels) {
            setPanels(prev => prev.map((p, i) => i === pp.idx ? { ...p, audioStatus: "generating" as const } : p));
            let success = false;

            for (let ac = 0; ac < audioChain.length && !success; ac++) {
              const { provider: tryP, voice: tryV, apiKey: tryK } = audioChain[ac];
              try {
                const res = await fetch("/api/audio", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ script: pp.narration, voice: tryV, provider: tryP, apiKey: tryK }) });
                if (res.ok) {
                  const blob = await res.blob();
                  if (blob.size > 0) {
                    audioBlobsRef.current[pp.idx] = blob; success = true;
                    setPanels(prev => { const u = prev.map((p, i) => i === pp.idx ? { ...p, audioUrl: URL.createObjectURL(blob), audioStatus: "done" as const } : p); saved.panels = u; savePipeline(saved); return u; });
                  }
                } else {
                  const e = await res.json().catch(() => ({}));
                  if (isAudioRateLimitError(e.error || "") && ac < audioChain.length - 1) continue;
                }
              } catch (err) {
            ...
            if (!success) setPanels(prev => prev.map((p, i) => i === pp.idx ? { ...p, audioStatus: "error" as const } : p));
            await new Promise(r => setTimeout(r, 1000));
          }

          // Merge all blobs for full player
          const allBlobs = audioBlobsRef.current.filter((b): b is Blob => b !== null);
          if (allBlobs.length > 0) {
            setAudioUrl(URL.createObjectURL(new Blob(allBlobs, { type: allBlobs[0].type || "audio/mpeg" })));
  ...
  }, [primaryLanguage, selectedTextModel, selectedImageModel, selectedAudioProvider, selectedAudioVoice]);
  ...
  const runPipeline = useCallback(async () => {
    ...
    if (!characterRef) {
      ...
      if (config) {
        ...
        } catch { console.log("Character sheet generation failed — continuing without"); logError("character", "Character sheet generation failed — continuing without character consistency"); }
      }
    }
    ...
    if (panelResult && panelResult.length > 0) {
      ...
      for (let i = 0; i < panelResult.length; i++) {
        ...
        // ── 3A: Generate AUDIO for this panel (auto-fallback through voice presets) ──
        if (audioKey && panelResult[i].audioStatus !== "done") {
          setPanels(prev => prev.map((p, idx) => idx === i ? { ...p, audioStatus: "generating" as const } : p));

          let audioSuccess = false;
          const audioChain = getAudioFallbackChain(selectedAudioProvider, selectedAudioVoice);

          for (let ac = 0; ac < audioChain.length && !audioSuccess; ac++) {
            const { provider: tryProvider, voice: tryVoice, apiKey: tryKey } = audioChain[ac];
            try {
              if (ac > 0) console.log(`Panel ${i+1} audio FALLBACK → ${tryProvider}/${tryVoice}`);
              else console.log(`Panel ${i+1} audio: ${tryProvider}/${tryVoice}`);

              const res = await fetch("/api/audio", {
                method: "POST", headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ script: panelResult[i].narration, voice: tryVoice, provider: tryProvider, apiKey: tryKey }),
              });
              if (res.ok) {
                const blob = await res.blob();
                if (blob.size > 0) {
                  ...
                  audioSuccess = true;
                  console.log(`Panel ${i+1} audio OK: ${(blob.size/1024).toFixed(0)}KB${ac > 0 ? ` (fallback ${tryProvider}/${tryVoice})` : ""}`);
                  const panelChars = panelResult[i].narration.length;
                  const ttsCost = (panelChars / 1000000) * 15;
                  logAI("audio", `Panel ${i+1} audio${ac > 0 ? ` (fallback → ${tryVoice})` : ""}`, tryProvider, `${tryProvider === "openai" ? "gpt-4o-mini-tts" : "gemini-tts"} / ${tryVoice}`, panelChars, 0, ttsCost);
                }
              } else {
                const e = await res.json().catch(() => ({}));
                ...
                logError("audio", `Panel ${i+1} audio failed`, errMsg, tryProvider, tryVoice);
              }
            } catch (err) {
        ...
        // ── 3B: Use Claude/AI to enhance image prompt — with character registry ──
          ...
          if (textConfig) {
            try {
              ...
              if (enhanceRes.ok) {
                ...
                // Save new character descriptions to the registry for future panels
                if (eData.newCharacters && eData.newCharacters.trim()) {
                  const newChars = eData.newCharacters.trim().split("\n").filter((l: string) => l.trim());
                  characterRef = characterRef + "\n" + eData.newCharacters.trim();
                  characterRefStore.current = characterRef;
                  console.log(`Panel ${i+1}: New characters added to registry: ${newChars.length} character(s)`);
                  for (const nc of newChars) { logInfo("character", `Panel ${i+1} — New character saved: ${nc.slice(0, 120)}`); }
                  logInfo("character", `Character registry updated: ${newChars.length} new character(s) added at panel ${i+1}`);
                  // Also save to localStorage so they persist across chapters
                  const charSheetKey = `ss_characters_${novelName.trim().toLowerCase().replace(/\s+/g, "_")}`;
                  try { localStorage.setItem(charSheetKey, characterRef); } catch {}
                }
              }
    ...
    setPhase("done");
    pipelineRunning.current = false;
  }, [hasScript, userScript, novelName, style, primaryLanguage, selectedTextModel, selectedImageModel, selectedAudioProvider, selectedAudioVoice]);
  ...
  // ── Retry ONLY failed/errored audio panels — keep successful ones, with auto-fallback ──
  const retryAudio = useCallback(async () => {
    ...
    // Find panels that need audio
    const failedPanels = panels.map((p, i) => ({ ...p, idx: i })).filter(p => p.audioStatus === "error" || p.audioStatus === "pending");
    if (failedPanels.length === 0) { pipelineRunning.current = false; setAudioStatus("done"); return; }

    const alreadyDone = panels.filter(p => p.audioStatus === "done").length;
    console.log(`Retry Audio: ${failedPanels.length} panels to retry (${alreadyDone} already done)`);

    // Build fallback chain from selected provider + saved presets
    const audioChain = getAudioFallbackChain(selectedAudioProvider, selectedAudioVoice);
    if (audioChain.length === 0) { setAudioStatus("error"); setError("No audio API key. Add in Settings."); pipelineRunning.current = false; return; }

    // Ensure audioBlobsRef is the right size
    if (audioBlobsRef.current.length !== panels.length) {
      const newBlobs = new Array(panels.length).fill(null);
      // Preserve existing blobs
      for (let i = 0; i < Math.min(audioBlobsRef.current.length, panels.length); i++) {
        if (audioBlobsRef.current[i]) newBlobs[i] = audioBlobsRef.current[i];
      }
      audioBlobsRef.current = newBlobs;
    }

    let newSuccess = 0;

    for (const pp of failedPanels) {
      setPanels(prev => prev.map((p, i) => i === pp.idx ? { ...p, audioStatus: "generating" as const } : p));
      ...
      for (let ac = 0; ac < audioChain.length && !success; ac++) {
        ...
        try {
          if (ac > 0) console.log(`Panel ${pp.idx+1} audio FALLBACK → ${tryProvider}/${tryVoice}`);
          else console.log(`Retrying panel ${pp.idx+1} audio: ${tryProvider}/${tryVoice}`);

          const res = await fetch("/api/audio", {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ script: pp.narration, voice: tryVoice, provider: tryProvider, apiKey: tryKey }),
          });

          if (res.ok) {
            const blob = await res.blob();
            if (blob.size > 0) {
              audioBlobsRef.current[pp.idx] = blob;
              newSuccess++;
              success = true;
              console.log(`Panel ${pp.idx+1} audio OK: ${(blob.size/1024).toFixed(0)}KB${ac > 0 ? ` (fallback ${tryProvider}/${tryVoice})` : ""}`);
              const panelChars = pp.narration.length;
              const ttsCost = (panelChars / 1000000) * 15;
              logAI("audio", `Retry panel ${pp.idx+1} audio OK${ac > 0 ? ` (fallback → ${tryVoice})` : ""}`, tryProvider, `${tryProvider === "openai" ? "gpt-4o-mini-tts" : "gemini-tts"} / ${tryVoice}`, panelChars, 0, ttsCost);
              setPanels(prev => {
                const u = prev.map((p, i) => i === pp.idx ? { ...p, audioUrl: URL.createObjectURL(blob), audioStatus: "done" as const } : p);
                const saved = loadPipeline(); if (saved) { saved.panels = u; savePipeline(saved); }
                return u;
              });
            }
          } else {
            const e = await res.json().catch(() => ({}));
            const errMsg = e.error || `HTTP ${res.status}`;
            // Rate limit? Try next fallback voice
            if (isAudioRateLimitError(errMsg) && ac < audioChain.length - 1) {
              logError("audio", `Retry panel ${pp.idx+1} rate limited on ${tryVoice}, trying fallback...`, errMsg, tryProvider, tryVoice);
              continue;
            }
            logError("audio", `Retry panel ${pp.idx+1} audio failed`, errMsg, tryProvider, tryVoice);
          }
        } catch (err) {
          const errMsg = err instanceof Error ? err.message : "Network error";
          if (isAudioRateLimitError(errMsg) && ac < audioChain.length - 1) { logError("audio", `Retry panel ${pp.idx+1} rate limited, trying fallback`, errMsg, tryProvider, tryVoice); continue; }
          logError("audio", `Retry panel ${pp.idx+1} audio error`, errMsg, tryProvider, tryVoice);
        }
      }

      if (!success) {
        setPanels(prev => prev.map((p, i) => i === pp.idx ? { ...p, audioStatus: "error" as const } : p));
      }

      // Merge all available blobs and update the full audio player
      const allBlobs = audioBlobsRef.current.filter((b): b is Blob => b !== null);
      if (allBlobs.length > 0) {
        const merged = new Blob(allBlobs, { type: allBlobs[0].type || "audio/mpeg" });
        setAudioUrl(URL.createObjectURL(merged));
    ...
    const totalDone = panels.filter(p => p.audioStatus === "done").length + newSuccess;
    const stillFailed = panels.length - totalDone;

    if (totalDone > 0) {
      setAudioStatus("done");
      if (script) addTTSUsage(script.length);
      const saved = loadPipeline(); if (saved) { saved.audioStatus = "done"; savePipeline(saved); }
      if (stillFailed > 0) setError(`Audio: ${totalDone}/${panels.length} panels OK. ${stillFailed} still failed — retry again.`);
    } else { setAudioStatus("error"); }

    pipelineRunning.current = false;
  }, [panels, selectedAudioProvider, selectedAudioVoice, script]);
  ...
  const retryImages = useCallback(async () => {
    ...
    setImageStatus("running");
    let charRef = characterRefStore.current;
    ...
    for (const pp of erroredPanels) {
      ...
      // Enhance prompt with character ref — uses growing character registry
      let finalPrompt = pp.imagePrompt;
      const textConfig = getScriptConfig(selectedTextModel);
      if (textConfig && charRef) {
        try {
          const enhanceRes = await fetch("/api/novel", {
            method: "POST", headers: { "Content-Type": "application/json" },
          ...
          if (enhanceRes.ok) {
            const eData = await enhanceRes.json();
            if (eData.imagePrompt) finalPrompt = eData.imagePrompt;
            // Accumulate new characters into registry
            if (eData.newCharacters && eData.newCharacters.trim()) {
              const newChars = eData.newCharacters.trim().split("\n").filter((l: string) => l.trim());
              charRef = charRef + "\n" + eData.newCharacters.trim();
              characterRefStore.current = charRef;
              for (const nc of newChars) { logInfo("character", `Retry panel ${pp.panel} — New character saved: ${nc.slice(0, 120)}`); }
              logInfo("character", `Character registry updated: ${newChars.length} new character(s) at retry panel ${pp.panel}`);
              const charSheetKey = `ss_characters_${novelName.trim().toLowerCase().replace(/\s+/g, "_")}`;
              try { localStorage.setItem(charSheetKey, charRef); } catch {}
            }
          }
        } catch {}
      }
      ...
      try {
        ...
        if (res.ok) {
          ...
          const e = await res.json().catch(() => ({}));
          logError("image", `Retry panel ${pp.panel} image failed`, e.error || "Failed", imgProvider, selectedImageModel);
    ...
    pipelineRunning.current = false;
  }, [panels, selectedImageModel, selectedTextModel]);
  ...
  // Auto-detect actual content language from panels (don't trust primaryLanguage — it may be stale)
  function detectPanelLanguage(): "english" | "hindi" {
    if (panels.length === 0) return "english";
    // Check first 5 panels for Devanagari characters
    const sample = panels.slice(0, 5).map(p => p.narration).join(" ");
    const devanagariCount = (sample.match(/[\u0900-\u097F]/g) || []).length;
    // If more than 20% of characters are Devanagari, content is Hindi
    return devanagariCount > sample.length * 0.15 ? "hindi" : "english";
  }

  const actualContentLang = panels.length > 0 ? detectPanelLanguage() : primaryLanguage;
  const secondLanguage = actualContentLang === "english" ? "Hindi" : "English";
  ...
  const generateSecondLang = useCallback(async () => {
    ...
    // Detect actual content language from panels, not from state
    const contentLang = detectPanelLanguage();
    const targetLang = contentLang === "english" ? "Hindi" : "English";
    const isTargetHindi = targetLang === "Hindi";

    const textConfig = getScriptConfig(selectedTextModel);
    const audioChain = getAudioFallbackChain(selectedAudioProvider, selectedAudioVoice);

    if (!textConfig) { setError("No AI key for translation"); pipelineRunning.current = false; setSecondLangStatus("idle"); return; }

    logInfo("system", `Second language generation started: ${contentLang} → ${targetLang} (${panels.length} panels)`);

    // Translation prompt — use simple Hindi when translating TO Hindi
    const translationSystemPrompt = isTargetHindi
      ? "You are a translator. Translate the following text to simple, everyday spoken Hindi (the kind people use in normal conversation). Output ONLY the Hindi translation.\n\nHINDI STYLE RULES:\n- Use SIMPLE conversational Hindi that everyone understands. Like how friends talk to each other.\n- Do NOT use Shudh Hindi, Sanskrit-heavy words, or complex literary Hindi.\n- Use common Hinglish words where natural (e.g., 'power' instead of 'shakti', 'attack' instead of 'aakraman', 'fight' instead of 'yuddh').\n- Keep character names in English/original language.\n- Keep the same dramatic tone and emotion.\n- Do NOT add labels, notes, or explanations.\n- Translate EVERY sentence. Do not skip anything.\n- Write Hindi in Devanagari script.\n- Do NOT add ellipsis (...), dashes, or [pause] markers."
      : "You are a translator. Translate the following text to English. Output ONLY the English translation, nothing else. No labels, no headers. Keep the same dramatic tone and emotion. Keep character names as-is. Translate EVERY sentence.";
    ...
    for (let i = 0; i < panels.length; i++) {
      ...
      try {
        ...
        if (translateRes.ok) {
          const tData = await translateRes.json();
          if (tData.reply) { translatedNarration = tData.reply; logAI("script", `Panel ${i+1} translated to ${targetLang}`, textConfig.provider, textConfig.model); }
        } else { logError("script", `Panel ${i+1} translation to ${targetLang} failed`, "API error", textConfig.provider, textConfig.model); }
      } catch { logError("script", `Panel ${i+1} translation error`, "Network error"); }

      // Generate audio in target language — with fallback chain
      let translatedAudioUrl: string | undefined;
      let translatedAudioStatus: "pending" | "generating" | "done" | "error" = "pending";

      for (let ac = 0; ac < audioChain.length; ac++) {
        const { provider: tryP, voice: tryV, apiKey: tryK } = audioChain[ac];
        try {
          const audioRes = await fetch("/api/audio", {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ script: translatedNarration, voice: tryV, provider: tryP, apiKey: tryK }),
          });
          if (audioRes.ok) {
            const blob = await audioRes.blob();
            if (blob.size > 0) {
              translatedAudioUrl = URL.createObjectURL(blob);
              translatedAudioStatus = "done";
              const panelChars = translatedNarration.length;
              logAI("audio", `${targetLang} panel ${i+1} audio OK${ac > 0 ? ` (fallback → ${tryV})` : ""}`, tryP, `${tryP === "openai" ? "gpt-4o-mini-tts" : "gemini-tts"} / ${tryV}`, panelChars, 0, (panelChars / 1000000) * 15);
              break; // Success — stop trying
            }
          } else {
            const e = await audioRes.json().catch(() => ({}));
            if (isAudioRateLimitError(e.error || "") && ac < audioChain.length - 1) { logError("audio", `${targetLang} panel ${i+1} rate limited on ${tryV}, fallback...`, e.error || "", tryP, tryV); continue; }
            logError("audio", `${targetLang} panel ${i+1} audio failed`, e.error || "Failed", tryP, tryV);
            translatedAudioStatus = "error";
          }
        } catch (err) {
          const errMsg = err instanceof Error ? err.message : "";
          if (isAudioRateLimitError(errMsg) && ac < audioChain.length - 1) { logError("audio", `${targetLang} panel ${i+1} rate limited, fallback...`, errMsg, tryP, tryV); continue; }
          logError("audio", `${targetLang} panel ${i+1} audio error`, errMsg, tryP, tryV);
          translatedAudioStatus = "error";
        }
      }
    ...
    const doneAudios = results.filter(r => r.audioStatus === "done").length;
    const failedAudios = results.filter(r => r.audioStatus === "error").length;
    logInfo("system", `Second language (${targetLang}) complete: ${results.length} panels translated, ${doneAudios} audio OK, ${failedAudios} audio failed`);
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <footer className="py-6 text-center"><p className="text-xs text-muted">REKVON Studio — Novel Video Creator.</p></footer>
    </div>
  );
}
