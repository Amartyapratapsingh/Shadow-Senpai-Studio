...
import { getTop10SystemPrompt, getTop10UserPrompt } from "@/lib/top10-prompts";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { categoryPrompt, customTopic, style, config, customCTA, durationMinutes } = body;
    ...
    if (!categoryPrompt && !customTopic) {
      return NextResponse.json(
        { error: "Select a category or enter a custom topic" },
        { status: 400 }
      );
    }

    const systemPrompt = getTop10SystemPrompt(
      style || "engaging-and-dramatic",
      customCTA,
      durationMinutes ? Number(durationMinutes) : undefined
    );
    const userPrompt = getTop10UserPrompt(categoryPrompt, customTopic);
